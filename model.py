...
class DAGCN(nn.Module):
    def __init__(self, args, dataset: DataSet):
        ...
        # 初始化行为感知调整参数（包含'all'）
        self.behavior_scale = nn.ParameterDict({
            beh: nn.Parameter(torch.ones(self.embedding_size))
    ...
    def gcn_propagate(self, with_augmentation=False):
        ...
        for post_beh in self.behaviors:
            ...
            for pre_beh in pre_behaviors:
                ...
                # 核心优化：根据前置行为类型动态调整嵌入
                scaled_pre_emb = pre_embedding * self.behavior_scale[pre_beh] + self.behavior_bias[pre_beh]
